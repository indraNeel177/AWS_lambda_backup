...
def lambda_handler(event, context=None):
    ...
    if year and not month and not day:
        where_caluse = """ where "nc"."year" ='{}' """.format(year)
    elif year and month and not day:
        where_caluse = """ where "nc"."year" ='{}' and  "nc"."month" ='{}'""".format(str(year), str(month))
    elif year and month and day:
        where_caluse = """ where "nc"."year" ='{}' and "nc"."month" ='{}' and "nc"."day" ='{}'""".format(year, month,
                                                                                                         day)
    query_for_flight_handled= """ select "date","equipment number" as Equipment_Number, "equipment type" as Equipment_Type,"equipment category" as Equipment_Category,"equipment ownertype" as Equipment_OwnerType,"noncompliancefor" as NonComplianceFor,"start time" as Start_Time,"end time" as End_Time,"start location" as Start_Location,"end location" as End_Location, "comments" as Comments, "speed limit (kmph)" as Speed_Limit, "actual speed" as Actual_Speed, "operator id" as Employee_Id, "operator name" as Employee_Name,"flight number" as "Flight_Number","flight date" as Flight_Date,"operation name" as Common_Operation_Name, "team supervisor name" as Team_Supervisor,"team supervisor id" as Team_Supervisor_Id, year, month,day  from non_compliance as nc {} """.format(where_caluse)
                ...
                if 'QueryExecution' in response and \
                    ...
                    if state == 'SUCCEEDED':
                        ...
                        try:
                            ...
                            if not df.empty:
                                df[["Common_Operation_Name","Flight_Number", "Actual_Speed"]] = df[["Common_Operation_Name", "Flight_Number", "Actual_Speed"]].apply(lambda y: y.apply(lambda x: str(x) if str(x) and str(x).lower() not in ["nan", "none"] else ""))
