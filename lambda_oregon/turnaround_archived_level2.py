...
def lambda_handler(event, context):
    ...
    select * from (
    select Airline,FlightType,destination_type,BayType,bodytype,date1,avg(turnaround_time) as turnaround_time,flightnumber_departure from (
 select t1.*,t2.Bodytype from (select Airline,FlightType,
 if(ToAirport_IATACode IN ('HYD','DEL','BLR','BOM'), 'Metro', 'Non-Metro') AS destination_type,
 if(Bay IN {connected_bay},'Connected','Remote') AS BayType,
aircraft, flightnumber_departure,
...
)as temp1 group by Airline,FlightType,destination_type,BayType,bodytype,date1,flightnumber_departure
    ) as n_temp1 where Airline='{Airline}'
...
aircraft,AircraftRegistration_Arrival, flightnumber_departure,
date(date_parse(On_Block_Time, '%Y-%m-%d %H:%i:%s')) as date1,
-- AS Off_Block_TimeR,
(DATE_DIFF('minute',date_parse(On_Block_Time, '%Y-%m-%d %H:%i:%s'),
if(date_parse(off_block_time,'%Y-%m-%d %H:%i:%s') is NULL, 
...
left join (sELECT Hexcode,BodyType,REPLACE(RegNo, '-', '') as RegNo FROM avileap_prod.flightmaster_parquet) as t2 on t1.AircraftRegistration_Arrival=t2.RegNo
    ...
    response = athena.start_query_execution(QueryString=turnaround_level_2_query,
    ...
    turnaround_level_2_raw_data = pd.concat([pd.read_csv(io.BytesIO(s3.get_object(Bucket=bucket, Key=i)['Body'].read()),
                                                         encoding="ISO-8859-1", error_bad_lines=False) for i in keys if
                                             i.endswith('csv')], ignore_index=True)
    ...
    if OpName == '4' or OpName == '13':
        turnaround_level_2_raw_data['FlightType'] = 'Domestic'
    turnaround_level_2_raw_data.turnaround_time.dropna()
    turnaround_level_2_raw_data = turnaround_level_2_raw_data.fillna("Unknown")
    print(turnaround_level_2_raw_data.head())
    turnaround_level_2_raw_data.turnaround_time = round(turnaround_level_2_raw_data.turnaround_time)

    turnaround_level_2_raw_data_json = turnaround_level_2_raw_data.to_json(orient='split')

    return {
        "response_code": 200,
        "level_2_all_flight_turnaround": json.loads(turnaround_level_2_raw_data_json)
    }
